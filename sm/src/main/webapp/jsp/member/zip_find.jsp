...
function selectnow() {
	...
	*/
	...
	opener.document.m.join_zip.value=zip1;
    //opener객체는 우편번호 공지창에서 본 회원가입창을 가리킨다. 
	/* 회원가입창에 첫번째 세자리 우편번호를 입력한다. */

	opener.document.m.join_zip2.value=zip2;

	/* 회원가입창에 두번째 세자리 우편번호를 입력한다. */

	opener.document.m.join_addr.value=addr2;

	/* 회원가입창에  주소를  입력한다. */

	opener.document.m.join_addr2.focus();
	//나머지 주소 입력창으로 포커스 이동
...
<table width="414" height="100" border="0" align="center" cellpadding="0" cellspacing="0">
 ...
 <tr height="30"> 
  <td bgcolor="f5ffea" align="center"><input type="text" name="dong"  size="10" maxlength="10">&nbsp;
  <input type="image"  src="images/m-i02.gif" width="69" height="19" align="absmiddle" onfocus="this.blur()"></td>
 </tr>
 <%//if(dong != null){ %>
 ...
 <tr> 
  <td bgcolor="f5ffea" height="30" align="center">
   ...
   <!-- items 속성에는 검색결과 주소값을 담고 있는 키값을 기입 , addr2 변수에서 주소값을 받아서 저장-->
             <c:set var="totaladdr" value="${addr2.zipcode}${addr2.add}" />   
    ...
    <option value="${totaladdr}">[${addr2.zipcode}]&nbsp;${addr2.add}</option>    
