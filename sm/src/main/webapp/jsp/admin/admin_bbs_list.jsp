...
<%-- method속성을 생략하면 기본으로 get방식 --%>
  <div id="abList_wrap">   
    <h2 class="abList_title">관리자 자료실 목록</h2>
 ...
 <a href="admin_bbs_cont.do?bbs_no=${list.bbs_no}&page=${page}&state=cont" 
 ...
 --%>
            ...
            <td align="center">${list.bbs_name}</td>
	  		...
	  		onclick="location='admin_bbs_cont.do?bbs_no=${list.bbs_no}&page=${page}&state=edit'">/
	  		<input type="button" value="삭제" class="input_b"
	  		onclick="if(confirm('정말로 삭제할까요?')==true){
	  		location='admin_bbs_cont.do?bbs_no=${list.bbs_no}&page=${page}&state=del'
	  		 	}else{
	  		 	return;}">
	  		</td>
          </tr>
	...
	<div id="list_paging">
	  ...
	  <c:if test="${(!empty find_field) && (!empty find_name)}">
			...
			<c:if test = "${page > 1 }">
			<a href="admin_bbs_list.do?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
			</c:if>	
			...
			<c:forEach  var="a" begin = "${startpage}" end = "${endpage}" step = "1">
                ...
                <c:if test = "${a != page}">
				 <a href="admin_bbs_list.do?page=${a}&find_field=${find_field}&find_name=${find_name}">[${a}]</a>&nbsp;
				</c:if>
			</c:forEach>	
			...
			<c:if test = "${page < maxpage }">
			 <a href="admin_bbs_list.do?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
			</c:if>
	      </c:if>
	  <!-- 검색후 페이징 끝 -->
    ...
    <c:if test="${(empty find_field) && (empty find_name)}">
			...
			<c:if test = "${page > 1 }">
			<a href="admin_bbs_list.do?page=${page-1}">[이전]</a>&nbsp;
			</c:if>	
			...
			<c:forEach  var="a" begin = "${startpage}" end = "${endpage}" step = "1">
                ...
                <c:if test = "${a != page}">
                <%--해당 쪽번호가 선택 안된 경우 실행 --%>
				 <a href="admin_bbs_list.do?page=${a}">[${a}]</a>&nbsp;
				</c:if>
			</c:forEach>	
			...
			<c:if test = "${page < maxpage }">
			 <a href="admin_bbs_list.do?page=${page+1}">[다음]</a>
			</c:if>
	    </c:if>
	    <!-- 검색전 페이징(쪽나누기) 끝 -->
	</div>
	...
	<div id="list_menu">
		...
		onclick="location='admin_bbs_write.do?page=${page}'" />	
